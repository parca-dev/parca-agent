...
func (p *CPU) ProcessReports() map[int]error {
	p.mtx.RLock()
	defer p.mtx.RUnlock()
	return p.processReports
}
...
func (p *CPU) report(lastError error, processReports map[int]error) {
	p.mtx.Lock()
	defer p.mtx.Unlock()
	...
	p.processReports = processReports
}
