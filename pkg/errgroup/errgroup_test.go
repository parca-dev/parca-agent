...
func ExampleGroup_justErrors() {
	...
	for _, url := range urls {
		...
		g.Go(context.TODO(), func(_ context.Context) error {
			// Fetch the URL.
...
func TestZeroGroup(t *testing.T) {
	...
	for _, tc := range cases {
		...
		for i, err := range tc.errs {
			err := err
			g.Go(context.TODO(), func(_ context.Context) error { return err })
...
func BenchmarkGo(b *testing.B) {
	...
	b.ReportAllocs()
	for i := 0; i < b.N; i++ {
		g.Go(ctx, func(_ context.Context) error { fn(); return nil })
	}
	g.Wait()
}
