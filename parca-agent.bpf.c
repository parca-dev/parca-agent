...
#define LOG(_pid, _msg)                                                       \
  {                                                                           \
    event_t e = { .pid = _pid, .message = _msg };                             \
    bpf_ringbuf_output (&logs, &e, sizeof (e), 0);                            \
  }
...
BPF_ARRAY (rips, u32, stack_unwind_instruction_t, 0xffffff); // MAX_ENTRIES
BPF_ARRAY (rsps, u32, stack_unwind_instruction_t, 0xffffff); // MAX_ENTRIES
