...
//
// It is a peer of arrowReporter rather than a wrapper around upstream's OTLP
// reporter, whose converter drops GPU origins and cannot express a memory
// profile's four sample types in one pprofile.Profile. Both limits live behind
// reporter/internal/pdata, which is unimportable, so the conversion is ours;
// see pprofile.go.
...
// ReportTraceEvent converts one trace event into a pprofile sample. Every
// origin the tracer can produce has a sample type, so unlike the arrow paths
// there is no schema branch.
...
// labels are resolved once and the builder lock is taken once. Each axis is
// its own sample type and so its own Profile.
