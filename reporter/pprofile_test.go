...
// TestPprofileCUDAFrame asserts the two values the ADR's per-kernel artifact key
// is derived from -- the cubin CRC and the mangled kernel name -- both survive
// the encoding, and that the frame type gating that lookup is present.
