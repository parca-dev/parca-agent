...
// reporterCounters are the metrics describing sample intake rather than a wire
// format, so every backend registers them. The arrow write counters stay on
// arrowReporter because they count WriteArrow calls, so those series are
// absent in OTLP mode.
