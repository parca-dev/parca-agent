...
// Attribute keys. These are the contract with the debuginfo pipeline and the
// profiling store: the build-id keys are what artifacts are keyed on, and
// profileFrameTypeKey is what gates per-frame-type symbolization (notably the
// "cuda-pc" path, whose artifact key is derived from the cubin build ID plus
// the kernel's mangled name).
//
// They match the semantic conventions upstream's OTLP reporter emits, spelled
// out here rather than pulled from the semconv module so a semconv bump cannot
// silently rename a key the backend joins on.
const (
...
// Sample types, one per origin plus the four memory axes.
//
// Where upstream's OTLP reporter already defines a name we use it (off_cpu,
// events) so a collector sees conventional profiles. Where it defines none --
// GPU and memory, which it cannot represent at all -- we keep the names the
// arrow backends use. The mixed vocabulary is deliberate and is the one place
// a query written against the arrow backend will not match this one: off-CPU is
// "wallclock" there and "off_cpu" here.
...
//
// The dictionary (strings, functions, mappings, locations, stacks, attributes)
// is global to the request, so interning happens once here regardless of how
// many resources or sample types the batch spans. reporter/internal/orderedset
// in the upstream module does the same job but is internal, so these are plain
// maps into the dictionary tables.
...
func (b *pprofileBuilder) internMapping(k mappingKey) int32 {
	...
	// The build-id attributes are the join key for symbolization. Emit the
	// typed ones when known and always the htlhash fallback, mirroring
	// upstream so a backend keying on either finds it.
...
// Those carry the process's build ID and executable path on the frame itself
// (FunctionName and SourceFile) because there is no mapping to hang them on.
// Reading them as a real function name and file -- which is what a generic
// frame classifier does -- would emit a function literally named after a build
// ID, so this is a separate path on purpose.
