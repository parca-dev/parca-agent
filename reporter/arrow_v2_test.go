...
func TestStacktraceDictBuilderV2_NullLinesForUnsymbolizedFrames(t *testing.T) {
	mem := memory.NewGoAllocator()
	builder := NewStacktraceDictBuilderV2(mem)
	defer builder.Release()

	// Native frame: no lines appended → lines should be null
	nativeFrame := libpf.Frame{
		Type:            libpf.NativeFrame,
		AddressOrLineno: 0x1000,
	}
	// Kernel frame: has lines → lines should be non-null
	kernelFrame := libpf.Frame{
		Type:            libpf.KernelFrame,
		AddressOrLineno: 0x2000,
		FunctionName:    libpf.Intern("do_syscall_64"),
	...
	frames.Append(&kernelFrame)

	hash := libpf.NewTraceHash(10, 20)
	builder.AppendStacktrace(hash, frames, appendLocation)
	...
	// The stacktrace is a ListView of Dict[Uint32, LocationStruct].
	// Drill into the location dictionary values to inspect the lines field.
	listView := arr.(*array.ListView)
	locDict := listView.ListValues().(*array.Dictionary)
	locStruct := locDict.Dictionary().(*array.Struct)

	// lines is the 5th field (index 4) in LocationTypeV2
	linesField := locStruct.Field(4)

	// Location 0 is the native frame → lines should be null
	require.True(t, linesField.IsNull(0), "native frame (no lines) should have null lines for async symbolization")

	// Location 1 is the kernel frame → lines should be non-null
	require.False(t, linesField.IsNull(1), "kernel frame (has lines) should have non-null lines")
}
