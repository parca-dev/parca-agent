...
func mainWithExitCode() flags.ExitCode {
	...
	// The go tracer is off unless asked for: Parca symbolizes native Go frames
	// server-side from uploaded debuginfo, so running the in-agent interpreter
	// too would pay for the same answer twice. --symbolize-go is for backends
	// with no symbolizer, where in-agent gopclntab resolution is the only way
	// Go frames get names.
	...
	// for Dash0 that is not the OTLP receiver, since the service issues signed
	// URLs and proxies large uploads -- while credentials and TLS still come
	// from the --remote-store-* flags. Empty reuses the remote-store conn.
